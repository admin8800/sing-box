...
change() {
    ...
    10)
        ...
        [[ $(grep -i "^admin8800$" <<<$is_servername) ]] && {
            err "你干嘛～哎呦～"
        }
        add $net
        ;;
...
get() {
    ...
    protocol)
        ...
        socks*)
            net=socks
            is_protocol=$net
            [[ ! $is_socks_user ]] && is_socks_user=admin8800
...
info() {
    ...
    ws | tcp | h2 | quic | http*)
        if [[ $host ]]; then
            ...
            [[ $is_protocol == 'vmess' ]] && {
                is_vmess_url=$(jq -c '{v:2,ps:'\"you-$net-$host\"',add:'\"$is_addr\"',port:'\"$is_https_port\"',id:'\"$uuid\"',aid:"0",net:'\"$net\"',host:'\"$host\"',path:'\"$path\"',tls:'\"tls\"'}' <<<{})
                ...
                is_url="$is_protocol://$uuid@$host:$is_https_port?encryption=none&security=tls&type=$net&host=$host&path=$path#you-$net-$host"
            }
            ...
            is_vmess_url=$(jq -c "{v:2,ps:\"you-${net}-$is_addr\",add:\"$is_addr\",port:\"$port\",id:\"$uuid\",aid:\"0\",net:\"$net\",type:\"$is_type\"$is_quic_add}" <<<{})
            is_url=vmess://$(echo -n $is_vmess_url | base64 -w 0)
        fi
        ;;
    ss)
        ...
        is_url="ss://$(echo -n ${ss_method}:${ss_password} | base64 -w 0)@${is_addr}:${port}#you-$net-${is_addr}"
    ...
    trojan)
        ...
        is_url="$is_protocol://$password@$is_addr:$port?type=tcp&security=tls&allowInsecure=1#you-$net-$is_addr"
        is_info_str=($is_protocol $is_addr $port $password tcp tls true)
        ;;
    hy*)
        is_can_change=(0 1 4)
        is_info_show=(0 1 2 10 8 9 20)
        is_url="$is_protocol://$password@$is_addr:$port?alpn=h3&insecure=1#you-$net-$is_addr"
    ...
    tuic)
        ...
        is_url="$is_protocol://$uuid:@$is_addr:$port?alpn=h3&allow_insecure=1&congestion_control=bbr#you-$net-$is_addr"
    ...
    reality)
        ...
        is_url="$is_protocol://$uuid@$ip:$port?encryption=none&security=reality&flow=$is_flow&type=$is_net_type&sni=$is_servername&pbk=$is_public_key&fp=chrome#you-$net-$is_addr"
        ;;
    direct)
    ...
    socks)
        ...
        is_url="socks://$(echo -n ${is_socks_user}:${is_socks_pass} | base64 -w 0)@${is_addr}:${port}#you-$net-${is_addr}"
        ;;
    esac
    ...
    if [[ $is_new_install ]]; then
        warn "首次安装请查看帮助。重要提示：输入 sing-box 打开管理菜单"
    fi
    if [[ $is_url ]]; then
        msg "------------- ${info_list[12]} -------------"
